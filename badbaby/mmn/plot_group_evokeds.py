...
df_mmn, df_cdi = rd.return_ford_mmn_dfs()
# Group by...
...
two_mos_subjects = df_mmn[mask].Subject_ID.values.tolist()
six_mos_subjects = df_mmn[~mask].Subject_ID.values.tolist()
...
low_ses_subjects = df_mmn[mask].Subject_ID.values.tolist()
high_ses_subjects = df_mmn[~mask].Subject_ID.values.tolist()
...
for group, nm in zip([two_mos_subjects, six_mos_subjects,
...
df_merged.to_csv(op.join(static_dir, csv_filename),
                 sep='\t', encoding='utf-8')
