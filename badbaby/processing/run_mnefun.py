...
from score import IN_NAMES, IN_NUMBERS, score

ecg_channel = dict((f"bad_{k}", v) for k, v in zip(df["id"], df["ecg"]))
...
params = mnefun.Params(
    ...
    proj_sfreq=250,
    decim=300.0,
    hp_cut=defaults.highpass,
    hp_trans="auto",
    lp_cut=defaults.lowpass,
    lp_trans="auto",
    bmin=tmin,
...
params.analyses = ["All", "Individual", "Oddball"]
params.out_names = [["All"], ["standard", "ba", "wa"], ["standard", "deviant"]]
params.out_numbers = [
    [1, 1, 1],  # Combine all trials
    [1, 2, 3],  # All conditions
    [1, 2, 2],  # oddball
]
params.must_match = [[], [0, 1, 2], [0, 1, 2]]
