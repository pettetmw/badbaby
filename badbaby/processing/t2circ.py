...
# %%
def t2circ_from_epo_tfr(epopath):
    # epopath : subject's epo file path
    if epopath[-3:] == 'fif':
        epo = mne.read_epochs(epopath)
        ...
        tfr_epo = mne.time_frequency.read_tfrs( epopath )[0]
    ...
    M = ntrl
    xest = np.mean(x,0) # marginal sample means
    yest = np.mean(y,0)
    xdev = x - np.tile(xest,(M,1,1,1)) # marginal sample deviations
    ydev = y - np.tile(yest,(M,1,1,1))
    Vi = np.sum( xdev**2 + ydev**2, 0 ) # pooled marginal variances
    Vg = ( xest**2 + yest**2 ) * M / 2 # variance attributable to sample means
