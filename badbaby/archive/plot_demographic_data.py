...
lp = defaults.lowpass
demographics = ['group', 'gender', 'age',
                'headSize', 'birthWeight',
                'ses', 'maternalEdu', 'paternalEdu',
                'nSibs']
grouping = ['group', 'gender']

# read in data
df_x, df_y = rd.return_dataframes('mmn')
...
df_x_grpby = df_x[demographics].groupby(grouping)
df_x_grpby.describe().to_csv(op.join(defaults.datadir,
                                     'MMN-sample_demographic_desc.csv'))
# repeated measures cohort
df_rm = df_x[df_x['simmInclude'] == 1]
df_rm.info()
df_rm_grpby = df_rm[demographics].groupby(grouping)
# write out RM descriptives table
print(df_rm_grpby.describe())
