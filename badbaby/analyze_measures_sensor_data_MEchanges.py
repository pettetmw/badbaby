...
# Mean Difference between SES 
def plot_diff(df, feature):
    tmp = df.groupby(['CDIAge', 'ses_group'])[feature].mean().reset_index()
    (tmp.groupby('CDIAge').first()[feature]-tmp.groupby('CDIAge').last()[feature]).plot(kind='bar')
    return
...
ages = np.arange(18, 31, 3)
f, axs = plt.subplots(len(ages), 1, sharey=True, sharex=True) #for each age
...
for ai, ax in enumerate(axs):
        #all M3L for each age and SES
        tmp1 = df[df.CDIAge == ages[ai]][df.ses_group == 'high']['M3L'].values
        tmp2 = df[df.CDIAge == ages[ai]][df.ses_group == 'low']['M3L'].values
        
        norm1 = norm.pdf(x,tmp1.mean(),tmp1.std())
        norm2 = norm.pdf(x,tmp2.mean(),tmp2.std())
        
        mean_new = tmp1.mean()-tmp2.mean()
        std_new = np.sqrt(tmp1.std()**2+tmp2.std()**2)
        norm_comb = norm.pdf(x, mean_new, std_new )
        
        #x = np.linspace(-20, 20, len(norm1))
        ax.plot(x,norm1, 'b', label= "High")
        ax.plot(x,norm2, 'r', label= "Low")
        ax.plot(x,norm_comb, 'g', label= "Diff")
        ax.title.set_text(ages[ai])
        
        conf_int = stats.norm.interval(0.95, loc=mean_new, scale=std_new/ np.sqrt(len(norm_comb)))
       
        for x1 in conf_int: 
            plt.axvline(x=x1, label='x = {}'.format(x1))
            plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3,
            #           ncol=2, mode="expand", borderaxespad=0.)
        
        #ax.vlines( mean_new - std_new * 1.96, ymin=0, ymax=0.5, color='green', label='95% CI' )
        #ax.vlines( mean_new + std_new * 1.96, ymin=0, ymax=0.5, color='green' )
        #ax.legend()
        
        #ax.title.set_text('PDFs for Low, High SES of 18')
